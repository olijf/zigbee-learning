...
from scapy.all import Packet, conf, sendp, AsyncSniffer
#from scapy.layers.lightlinkzbee import *
from scapy.layers.zigbee import *
from util.wpan_interface import Phy
from patch.lightlinkzbee import *
...
class Transceiver:
    ...
    def send_and_receive(
        frame: Packet,
        process_response_func: Callable[[Packet], bool]=lambda x: x.haslayer(ZLLScanResponse),
        chan: int=11,
        phy: Phy=None,
        iface: str = "wpan0",
    ...
    @staticmethod
    def receive(process_response_func: Callable[[Packet], bool]=lambda x: x.haslayer(ZLLScanResponse), chan: int=11, phy: Phy=None, iface: str = "wpan0", sleep_time: float = 1.0) -> Tuple[Packet, int]:
        """
        ...
        if not sleep_time < 0.1:
            sniffer.stop()
            print(f"Received {len(sniffer.results)} packets")
            return_answer = Transceiver.process_sniffer_results(sniffer.results, process_response_func)
