...
if __name__ == '__main__':
    ...
    key = NWK_KEY
    PHILIPS_TARGET = bytes.fromhex("00:17:88:01:04:25:bc:e2".replace(":", ""))
    frames = rdpcap('run-met-niek-zn-lamp1.pcap')
    frames[204].show()
    frames[205].show()
    frames[209].show()
    #extended_source = frames[218].ext_src.to_bytes(8, 'big')
    extended_source = frames[207].ext_src.to_bytes(8, 'big')
    print(f"extended_source = {extended_source.hex(':')}")
    print(f"key = {key.hex(' ')}")
    payload, succes = CryptoUtils.zigbee_packet_decrypt(key, frames[209], extended_source)
    if succes:
